...
public class DataLogger {
    ...
    public DataLogger(String fileName, Telemetry telemetry, boolean incrementSerialNumber) {
        this.telemetry = telemetry;
        
        // src and any subfolder contents appear in OnBot Java (left side).
        // .txt files allow data display in OnBot Java.  Download as .csv files.
        
        new File(directoryPath).mkdir();  // create Datalogs folder if needed

        SerialNumber sn = new SerialNumber(telemetry);
        if (incrementSerialNumber) {
            sn.incrementSerialNumber();
        }
        int fileSerialNumber = sn.getSerialNumber();

        String filePath = directoryPath + "/" + fileName + "_" + String.format("%04d", fileSerialNumber) + ".txt";
